...
class PokemonHandler:
    ...
    def store(self):
        stats = StatSet(**{ name:self.generate_stat_object(value) for (name,value) in self.stats } )
        stats.save()
    ...
    def generate_stat_object(self, stat):
    ...
    def format_stats(self, data):
        return [(s['stat']['name'].replace('-', '_'), s) for s in data["stats"]]
